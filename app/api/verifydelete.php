...
try {
    ...
    if ($inputKey === $config['master']) {
        ...
        if (empty($fileData['del_key_hash'])) {
            ...
            // ハッシュ化されたキーとの照合
            if (!empty($inputKey)) {
                $isValidAuth = SecurityUtils::verifyPassword($inputKey, $fileData['del_key_hash'], $config['session_salt']);
            }
        }
    }
    ...
    if (!$isValidAuth) {
        ...
        $responseHandler->authError('削除キーが正しくありません。');
    }
    ...
    if (!$tokenStmt->execute($tokenData)) {
        $responseHandler->error('TOKEN_GENERATION_ERROR', '削除トークンの生成に失敗しました。');
    }
